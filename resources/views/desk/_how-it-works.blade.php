...
<div x-data="{ open: false }" class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
    <button type="button" @click="open = ! open" :aria-expanded="open ? 'true' : 'false'"
        ...
        <span class="min-w-0 flex-1">
            <span class="block text-sm font-bold">How it works, from request to delivery</span>
            <span class="mt-0.5 block text-xs text-slate-500">{{ count($steps) }} stages and what is expected from you at each one.</span>
        </span>
    ...
    <div x-show="open" x-cloak class="border-t border-slate-100 p-5 dark:border-slate-800">
        ...
        <div class="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900 dark:bg-amber-950/40">
            <p class="text-sm font-semibold text-amber-900 dark:text-amber-200">One deadline belongs to you</p>
            <p class="mt-1 text-sm leading-6 text-amber-800 dark:text-amber-200/90">
                When ITD finishes something only you can validate, it appears under
                <span class="font-semibold">Waiting on me</span>. You have
                <span class="font-semibold">{{ $window }} days</span> to respond. Without a response,
                your request is cancelled and the capacity slot moves to the next request. Saying
                “this still needs changes” is always better than not responding.
            </p>
        </div>

        @if ($writing !== [])
            <div class="mt-6">
                <p class="text-sm font-semibold">How to write a request that is easy to assess</p>
