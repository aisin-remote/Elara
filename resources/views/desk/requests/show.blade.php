...
@section('content')
        ...
        <section class="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 dark:border-amber-900/60 dark:bg-amber-950/30">
            ...
            @can('resubmit', $request)
                <form method="POST" action="{{ route('desk.requests.resubmit', $request) }}" class="mt-5 space-y-4">
                    @csrf
                    <div><x-label for="problem">Apa masalahnya sekarang?</x-label><x-textarea id="problem" name="problem" rows="4" required>{{ old('problem', $request->problem) }}</x-textarea><x-field-error name="problem" /></div>
                    <div><x-label for="desired_outcome">Seperti apa hasil yang Anda harapkan?</x-label><x-textarea id="desired_outcome" name="desired_outcome" rows="4" required>{{ old('desired_outcome', $request->desired_outcome) }}</x-textarea><x-field-error name="desired_outcome" /></div>
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-[1fr_300px] lg:items-start">
        <section class="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            <div>
                <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-400">Masalahnya</h2>
            ...
            <div class="border-t border-slate-100 pt-5 dark:border-slate-800">
                <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-400">Hasil yang diharapkan</h2>
        ...
        <div class="space-y-6">
            ...
            @can('withdraw', $request)
                <form method="POST" action="{{ route('desk.requests.withdraw', $request) }}" onsubmit="return confirm('Withdraw this request?')">
                    @csrf
                    <x-button variant="secondary" class="w-full">Tarik permintaan</x-button>
                </form>
            @endcan
        </div>
    </div>
