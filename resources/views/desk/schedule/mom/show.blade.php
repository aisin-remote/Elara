...
@section('content')
    <div class="flex flex-wrap items-start justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2"><x-badge :tone="match($meetingMinute->publication_status) { App\Enums\MeetingMinutePublicationStatus::DRAFT => 'warning', App\Enums\MeetingMinutePublicationStatus::LOCKED => 'slate', default => 'success' }">{{ $meetingMinute->publication_status->label() }}</x-badge>@can('update', $meetingMinute)<x-link-button href="{{ route('desk.schedule.mom.edit', [$requesterWorkspace, $meetingMinute->public_id]) }}" variant="secondary">Edit MOM</x-link-button>@endcan<x-link-button href="{{ route('desk.schedule.index', $requesterWorkspace) }}" variant="secondary">Back to Schedule</x-link-button></div>
    </div>
