<section class="mt-6 rounded-2xl border border-orbit-200 bg-orbit-50/50 p-5 dark:border-orbit-900 dark:bg-orbit-950/20">
    <div class="flex items-start gap-3">
        ...
        <div>
            <h2 class="font-bold">Your department decision is required</h2>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">Confirm that the need and expected benefit are worth sending to ITD.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ $request instanceof App\Models\ProjectRequest
        ...
        <div class="grid gap-2 sm:grid-cols-3">
            @foreach ([['approve', 'Approve'], ['needs_info', 'Request more information'], ['reject', 'Reject']] as [$value, $label])
        ...
        <div x-show="decision !== 'approve'" x-cloak>
            <x-label for="department_note">Reason or information required</x-label>
            <x-textarea id="department_note" name="note" rows="3" placeholder="Explain what the requester needs to update.">{{ old('note') }}</x-textarea>
