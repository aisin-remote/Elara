<dialog id="new-task-dialog" class="m-0 h-full max-h-none w-full max-w-none bg-transparent p-0 backdrop:bg-slate-950/45 sm:m-auto sm:h-auto sm:max-h-[90vh] sm:w-[720px] sm:rounded-3xl" x-ref="taskDialog" @if($errors->any() || request()->boolean('create')) open @endif>
    <div class="h-full overflow-y-auto bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-7 dark:bg-slate-900">
        ...
        @include('app.tasks._form', ['formPrefix' => 'new-task', 'defaultStatus' => $statuses->first(), 'task' => null])
    </div>
</dialog>
