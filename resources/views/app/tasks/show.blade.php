...
@section('content')
    <div class="flex flex-wrap items-start justify-between gap-4"><div><a href="{{ $task->project->taskListUrl() }}" class="text-sm font-semibold text-slate-500 hover:text-orbit-700">← {{ $task->project->isPersonal() ? 'My tasks' : $task->project->name }}</a><div class="mt-3 flex flex-wrap items-center gap-3"><span class="size-2.5 rounded-full" style="background:{{ $task->status->color }}"></span><h2 class="text-2xl font-bold tracking-tight">{{ $task->title }}</h2><span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold dark:bg-slate-800">{{ $task->priority->label() }}</span></div></div><div class="flex gap-2">@can('update',$task)<form method="POST" action="{{ route('internal.tasks.duplicate',$task) }}">@csrf<x-button variant="secondary">Duplicate</x-button></form>@endcan @can('delete',$task)<form method="POST" action="{{ route('internal.tasks.destroy',$task) }}" onsubmit="return confirm('Archive this task?')">@csrf @method('DELETE')<input type="hidden" name="return_to" value="{{ $task->project->taskListUrl() }}"><x-button variant="danger">Archive</x-button></form>@endcan</div></div>
